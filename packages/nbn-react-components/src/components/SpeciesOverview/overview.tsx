...
import { TDataObjectSchema } from '../../shared/hooks/nbn-atlas-api/useInfoCards';
import { TKvpValueSchema, TRedListSchema } 
    from '../../shared/hooks/nbn-atlas-api/useRedListCards';
...
export function InfoCards( { dataObjects } : IInfoCard): React.JSX.Element {

    const data = useMemo(() =>transformData(dataObjects), [dataObjects]);
    // Initialise all InfoCards to closed state.
            ...
            <AccordionBody className={styleAccordionBody}>
                ...
                {(datum.hasHomepage) ? 
                    (<span className='text-blue-500 hover:underline'>
                        <a href={datum.homepage} target='_blank' 
                            rel='noreferrer'>
                          {datum.providedBy}
                         </a>  
                     </span>) : 
          ...
          <AccordionBody className={styleAccordionBody}>
            <div className='flex'>
              <ul className='list-disc pl-5'>
                ...
                {getLI(linkEOL, 'Enclopaedia of Life')}
