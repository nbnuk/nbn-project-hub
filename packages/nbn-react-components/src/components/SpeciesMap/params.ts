...
export class Params {
    ...
    // -------------------------------------------------------------------------
    /** Set the boundary coords for the current centroid point and radius.
     */ 

    calcBoundaryFromRadius(): void {
        // Ensure we're in point mode rather than TVK mode.
        if (!this.explore) {
            return
        }
        const loc = this.explore.location;
        // Convert radius in km to offsets in degrees
        const latOff = loc.radius / 110.574;
        const lonOff = loc.radius / (111.320 * Math.cos(loc.latitude));
        const lll = L.latLng(loc.latitude-latOff, loc.longitude-lonOff);
        const ull = L.latLng(loc.latitude+latOff, loc.longitude+lonOff);
        this.bounds = L.latLngBounds(lll, ull);         
    }
    // -------------------------------------------------------------------------
    ...
    getPointUrl(): string {
        ...
        const baseUrl = 'https://records-ws.nbnatlas.org/ogc/wms/reflect?q=*:*&fq=occurrence_status:present';
        let queryUrl: string;
        // Are we in explore mode or TVK mode?...
        if (this.explore) {
            // Handle unique case of 'ALL_SPECIES'.
            const name = (this.explore.name === 'ALL_SPECIES') ? '*' : this.explore.name;
            queryUrl = (this.explore.isGroup) ? 
                `&fq=species_group:${name}` :
                `&fq=taxon_name:${name}`;
            queryUrl += `&lat=${this.explore.location.latitude}&lon=${this.explore.location.longitude}&radius=${this.explore.location.radius}`;
            this.calcBoundaryFromRadius();
        } else {
            queryUrl = `&fq=lsid:${this.tvk}`;
        }
    ...
    getWmsLayers(): TLayersWMS {
        ...
        if (this.query.length > 0) {
            ...
            wmsLayers['Accepted'] = this.getWmsLayerPoint(true, Colour.orange);
            wmsLayers['Unconfirmed'] = this.getWmsLayerPoint(false, Colour.lightorange);
        } else {
