...
Fix (nbn-react-components)

From the repo root (recommended), run:

yarn workspace @nbnuk/nbn-react-components remove \
...
Then add the packages you do want on Storybook 10 (framework + docs):

yarn workspace @nbnuk/nbn-react-components add -D \
storybook@^10.1.10 \
@storybook/react-vite@^10.1.10 \
@storybook/addon-docs@^10.1.10


(Storybook 10.1.10 is current on npm for storybook and @storybook/react-vite.)
npmjs.com
+2
npmjs.com
+2

Update your .storybook/main.*

In packages/nbn-react-components/.storybook/main.ts (or .js), remove any references to the removed addons (especially @storybook/addon-essentials, @storybook/addon-links, @storybook/addon-interactions, @storybook/blocks).

A safe minimal config looks like:
